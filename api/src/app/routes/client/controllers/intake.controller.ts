...
export const updateApproveStatus = async (ctx: Koa.Context) => {
  try {
    ...
    await updateIntake(ctx.params.id, { status: 'approved', projectId: projectData.id, approverUserId: auth.userId });
    ctx.body = {
...
const validateIntakeForm = (intake: IProjectIntake) => {
  ...
  if (intake.client) {
    ...
    if (client.responsibilityCenter && client.responsibilityCenter.toString().length !== 5) {
